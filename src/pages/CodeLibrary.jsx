...
export default function CodeLibrary() {
  ...
  const filteredDx = diagnoses.filter(d =>
    !q || d.code?.toLowerCase().includes(q) || d.description?.toLowerCase().includes(q)
  );
    ...
    <div className="space-y-4">
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-sm text-muted-foreground">Your office's standard procedures and diagnoses used in billing</p>
        </div>
      </div>

      {/* Tabs */}
      <div className="flex gap-1 bg-muted rounded-lg p-1 w-fit">
        {[["procedures", "Procedure Codes"], ["diagnoses", "Diagnosis Codes (ICD-10)"]].map(([key, label]) => (
          <button key={key} onClick={() => { setTab(key); setSearch(""); setEditingProc(null); setEditingDx(null); }}
