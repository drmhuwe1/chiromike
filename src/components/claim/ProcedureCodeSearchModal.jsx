...
export default function ProcedureCodeSearchModal({ onSelect, onClose }) {
  const [codes, setCodes] = useState([]);
  const [search, setSearch] = useState("");
  const [loading, setLoading] = useState(true);

  useEffect(() => {
    base44.entities.ProcedureCode.filter({ active: true }, "-updated_date", 200).then(data => {
      setCodes(data);
      setLoading(false);
    });
  }, []);
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40 p-4">
      <div className="bg-card border border-border rounded-2xl shadow-2xl w-full max-w-2xl p-6 space-y-4 max-h-[80vh] overflow-y-auto">
        <div className="flex items-center justify-between">
          <h2 className="text-lg font-bold">Select Procedure Code</h2>
          ...
          <div className="space-y-1 max-h-96 overflow-y-auto">
            {filtered.map(code => (
              ...
              >
                ...
                {code.default_charge && (
                  <div className="text-xs text-primary">${code.default_charge.toFixed(2)}</div>
                )}
              </button>
            ))}
            {filtered.length === 0 && (
